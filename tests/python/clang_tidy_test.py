...
def test_clang_tidy_command(
    mocker, compile_commands, tmp_path, read_json_db, all_at_once, returncode, stdout, error_msg
):
...
def test_clang_tidy_main(mocker):
    run = mocker.patch('cmake_pc_hooks.clang_tidy.ClangTidyCmd.run')

    argv = ['clang-tidy', 'file.txt']

    clang_tidy.main(argv)
